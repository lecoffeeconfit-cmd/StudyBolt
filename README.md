...
The mobile build also carries the public processor route in `app.json` as a last-resort fallback, so an EAS build does not silently lose uploads when the `EXPO_PUBLIC_*` values were not copied into its environment. A configured `EXPO_PUBLIC_STUDYBOLT_PROCESSOR_URL` or `EXPO_PUBLIC_SUPABASE_URL` still takes precedence.
