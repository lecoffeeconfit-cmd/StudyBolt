...
The included `supabase/functions/studybolt-process-document/index.ts` first receives multipart form fields `file`, `courseName`, `documentType`, `extension`, and `action=extract`. It uploads/reads the source once and returns a Study Pack shell containing the normalized source and outline. The app persists that normalized source locally, opens the pack immediately, and sends independent JSON `action=generate` requests for Simplified Notes, Detailed Notes, Key Concepts, Flashcards, Audio Summary, and Practice Quiz. A failed material can therefore be retried without uploading or extracting the document again. Older clients that omit `action=extract` retain the legacy one-response contract.
...
The mobile build carries the self-hosted VPS processor route in `app.json` and in the client as a final built-in fallback, so an EAS build does not silently route uploads to a hosted Supabase project when the `EXPO_PUBLIC_*` values were not copied into its environment. `EXPO_PUBLIC_STUDYBOLT_PROCESSOR_URL` takes precedence, followed by the manifest route and then the same VPS route.

The function does not retain the uploaded source file, deletes the temporary provider file immediately after extraction, and uses `store: false` for every model response. The normalized source and per-material status live in the existing local AsyncStorage Study Pack record; no database migration is required. Add production rate limiting or require a user JWT before exposing a public guest-upload endpoint. A custom endpoint must keep uploads private and implement both the extraction and per-material JSON contracts. Both `notes` and `detailedNotes` must cover every source-outline section and retain a source reference for every chunk. Simplified notes are the fast layer: a condensed, one-sentence organizing summary, a short set of core ideas, one key relationship, and retrieval prompts. Detailed notes are intentionally not a second copy of that summary: they preserve most meaningful source claims and wording in hierarchical subsections, explicitly connect related ideas, include grounded examples when the source supports them, and end with self-explanation or retrieval prompts. Content must remain source-grounded rather than filling apparent gaps with invented facts. StudyBolt uses the generated notes and flashcards to build a cumulative test whose prompts are separate from the practice-quiz pool. Malformed or incompletely covered responses are rejected by the client, and legacy/shared packs are normalized into distinct layers on load.
